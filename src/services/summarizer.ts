...
const INVALID_FRAGMENT_START =
  /^(?:["'”’」)\],.，;:·…\-\–\—]+|(?:은|는|이|가|을|를|에|에서|에게|으로|로|와|과|의|도|만|부터|까지)\s|(?:때문에|덕분에|관해|관련해|중심으로|기반으로|통해|따르면|대해|위해)\s|[가-힣A-Za-z0-9%]+(?:하면서|되면서|하며|하고|했고|했으며|라는|라고|이라며|면서|는데|지만|거나|도록|기에)\s)/
